...
//------------- PDF -------------------
	...
	function generatePdf(pdf){	
		...
		var url = path.join('http://localhost:1337/', 'print');
		var filePath = pdfFolderPath+'/'+date+'-'+chapterFolder+'.pdf';
		var widthPx = 29.7 * 37.8;
    var heightPx = 21 * 37.8;
	  ...
	  '--local-to-remote-url-access=yes'
		  ...
		  ph.createPage().then(function(page) {
		  	page.open(url)
		  	.then(function(){
		  		page.property('paperSize', {width: widthPx, height: heightPx, orientation: 'landscape'})
		  		.then(function() {
